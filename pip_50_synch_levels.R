...
synch_levels_data_pip = readxl::read_excel(file.path(data_path, "synch_levels.xlsx")) %>% 
...
year_dummy_names = synch_levels_data_pip %>% 
...
ar.synch.levels.pip50.1 = stats::lm(synch ~ country  + draghisynch_lag1+ draghisynch_lag2+ recsynch_lag1 +
...
ar.synch.levels.pip50.2 = stats::lm(synch ~  country  + draghiinflation_lag3+ draghiinflation_lag4+ draghisynch_lag1+
...
ar.synch.levels.pip50.3 = stats::lm(synch ~ country + draghisynch_lag1 +draghisynch_lag2+ recsynch_lag1 +recsynch_lag2+
...
ar.synch.levels.pip50.4 = stats::lm(synch ~  country  + draghisynch_lag1+ draghisynch_lag2 +draghisynch_lag3+ recsynch_lag1 +
...
ar.synch.levels.pip50.5 = stats::lm(synch ~ country +draghisynch_lag1 +draghisynch_lag2 +draghisynch_lag3+ recsynch_lag1+
...
ar.synch.levels.pip50.6 = stats::lm(synch ~ country + draghisynch_lag1+ draghisynch_lag2+ draghisynch_lag3+ recsynch_lag1+
                               recsynch_lag2 +synch_lag1 +zlbsynch_lag1+ zlbsynch_lag2+year_2002+ year_2003+ year_2004+ year_2005+ year_2006+ year_2007+
...
ar.synch.levels.pip50.7 = stats::lm(synch ~ country +draghisynch_lag1+ draghisynch_lag2+ draghisynch_lag3+ recsynch_lag1+
                               recsynch_lag2+ synch_lag1+ zlbsynch_lag1+ zlbsynch_lag2+year_2002+ year_2003+ year_2004+ year_2005+ year_2006+ year_2007+
                   year_2008+ year_2009+ year_2010+ year_2011+ year_2012+ year_2013+ year_2014+ year_2015+ year_2016+ year_2017 +year_2018+year_2019+year_2020, data = synch_levels_data_pip %>% 
                   dplyr::mutate(date = zoo::as.Date(date)) %>% 
                   dplyr::filter(date <= zoo::as.Date("2020-04-01"))) 

ar.synch.levels.pip50.8 = stats::lm(synch ~ country + draghisynch_lag1+ draghisynch_lag2+ draghisynch_lag3+ recsynch_lag1+
                               recsynch_lag2+ synch_lag1 +zlbsynch_lag1 +zlbsynch_lag2+year_2002+ year_2003+ year_2004+ year_2005+ year_2006+ year_2007+
                   year_2008+ year_2009+ year_2010+ year_2011+ year_2012+ year_2013+ year_2014+ year_2015+ year_2016+ year_2017 +year_2018+year_2019+year_2020, data = synch_levels_data_pip %>% 
                   dplyr::mutate(date = zoo::as.Date(date)) %>% 
...
f_synch_index = base::which(base::colnames(synch_levels_pip_forecast_data)=="f_synch" )
...
for (mm in 1:dim(synch_levels_pip_forecast_data)[1]) {
  ...
  if (mm %in% seq(2,135,9)) {
    ...
    synch_levels_pip_forecast_data[mm,f_synch_index] =
      stats::predict(ar.synch.levels.pip50.1, newdata = synch_levels_pip_forecast_data[mm,c("draghisynch_lag1", "draghisynch_lag2",
  ...
  if (mm %in% seq(3,135,9)) {
    ...
    synch_levels_pip_forecast_data[mm,f_synch_index] =
      stats::predict(ar.synch.levels.pip50.2, newdata = synch_levels_pip_forecast_data[mm,c("draghiinflation_lag3", "draghiinflation_lag4", "draghisynch_lag1",
  ...
  if (mm %in% seq(4,135,9)) {
    ...
    synch_levels_pip_forecast_data[mm,f_synch_index] =
      stats::predict(ar.synch.levels.pip50.3, newdata = synch_levels_pip_forecast_data[mm,c("draghisynch_lag1", "draghisynch_lag2", "recsynch_lag1",
  ...
  if (mm %in% seq(5,135,9)) {
    ...
    synch_levels_pip_forecast_data[mm,f_synch_index] =
      stats::predict(ar.synch.levels.pip50.4, newdata = synch_levels_pip_forecast_data[mm,c("draghisynch_lag1", "draghisynch_lag2", "draghisynch_lag3",
  ...
  if (mm %in% seq(6,135,9)) {
    ...
    synch_levels_pip_forecast_data[mm,f_synch_index] =
      stats::predict(ar.synch.levels.pip50.5, newdata = synch_levels_pip_forecast_data[mm,c("draghisynch_lag1", "draghisynch_lag2", "draghisynch_lag3", "recsynch_lag1",
  ...
  if (mm %in% seq(7,135,9)) {
    ...
    synch_levels_pip_forecast_data[mm,f_synch_index] =
      stats::predict(ar.synch.levels.pip50.6, newdata = synch_levels_pip_forecast_data[mm,c("draghisynch_lag1", "draghisynch_lag2", "draghisynch_lag3", "recsynch_lag1",
  ...
  if (mm %in% seq(8,135,9)) {
    ...
    synch_levels_pip_forecast_data[mm,f_synch_index] =
      stats::predict(ar.synch.levels.pip50.7, newdata = synch_levels_pip_forecast_data[mm,c("draghisynch_lag1", "draghisynch_lag2", "draghisynch_lag3",
  ...
  if (mm %in% seq(9,135,9)) {
    ...
    synch_levels_pip_forecast_data[mm,f_synch_index] =
      stats::predict(ar.synch.levels.pip50.8, newdata = synch_levels_pip_forecast_data[mm,c("draghisynch_lag1", "draghisynch_lag2", "draghisynch_lag3",
                                                                                     "recsynch_lag1", "recsynch_lag2", "synch_lag1", "zlbsynch_lag1",
                                                                                     "zlbsynch_lag2", "country", year_dummy_names)])
...
base::cat(dir.results(synch_levels_pip_forecast_data), "\n")
base::cat(dir.results(synch_levels_pip_forecast_data.piigs), "\n")
base::cat(dir.results(synch_levels_pip_forecast_data.nopiigs), "\n")

# Print the forecast results without 2020 (part of Table A.5 in the paper)

base::cat(dir.results(without.2020(synch_levels_pip_forecast_data)), "\n")
base::cat(dir.results(without.2020(synch_levels_pip_forecast_data.piigs)), "\n")
base::cat(dir.results(without.2020(synch_levels_pip_forecast_data.nopiigs)), "\n")